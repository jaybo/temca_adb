...
''' Analyzes TEMCA raw image tiles using matcher data from metafile.
...
def analyze_results(args_col_row):
    ...
    if args.config_file:
        ...
        try:
            cfg = yaml.load(config_file_handle)["tile_qc"]
        except Exception as e:
        ...
        if "max_mean_focus_std_dev" in cfg:
            focus_std = np.std(np.nonzero(args.raw_focus))
        ...
        if actual_centroid and "aperture_centroid" in cfg and "max_centroid_shift" in cfg:
...
def save_images(args_col_row):
    ...
    args = args_col_row[2]
    prop_bag = args_col_row[3]
    if getattr(args, "image_directory"):
        try:
        ...
        for args in args_col_row[0:2]:
            if args:
                for im_type in [('f', 'focus'), ('s', 'std_dev'), ('d', 'distance'), ('x', 'x_offset'), ('y', 'y_offset'), ('q', 'quality')]:
                    ...
                    s = os.path.join(args.image_directory, s)
                    cv2.imwrite(s, prop_bag.im_out)
...
def display_neighborhood(args_col_row):
    ...
    while not prop_bag.quit:
        ...
        if place_using_matcher:
            ...
            for match_quality in [3, 2, 0]:
                for nr in list(range(0, neighbors+1)) + list(reversed(range(-neighbors, 1))):
                    for nc in list(range(0, neighbors+1)) + list(reversed(range(-neighbors, 1))):
...
def main(args):
    parent_parser = argparse.ArgumentParser(description='TEMCA raw tile analyzer, matches from metadata.',
    ...
    parent_parser.add_argument('directory', help='the directory to process.',
                               metavar="", nargs='?', default=r'D:\data\jay7\000000\0')
    # parent_parser.add_argument('directory', help='the directory to process.', nargs='?', default=r'd:\data\001047\0')
    # parent_parser.add_argument('directory', help='the directory to process.', nargs='?', default=r'Z:\1x1mm_data\TEMCA3\295434_5LC_0064_reimaging_01\001047\0')
    # parent_parser.add_argument('directory', help='the directory to process.', nargs='?', default=r'Z:\Network_Anatomy\DATA\perAnimal\243774_baylor_8973\243774_blocks\243774_7R\243774_7R_SID_02_redo_102\0\5') #phase 1 data Temca1
    # parent_parser.add_argument('directory', help='the directory to process.', nargs='?', default=r'Z:\1x1mm_data\TEMCA2\295434_5LC_0077_ASG_07\000114\0') #typewriter
    # parent_parser.add_argument('directory', help='the directory to process.', nargs='?', default=r'Z:\1x1mm_data\TEMCA3\295434_5LC_0064_reimaging_04_Type_writer_test\001050\0') #typewriter
